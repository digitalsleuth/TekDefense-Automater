...
class Parser(object):
    ...
    def __init__(self, desc, version):
        ...
        self._parser.add_argument(
            "target",
            help="List one IP Address (CIDR or dash notation accepted), URL or Hash to query or pass the filename of a file containing IP Address info, URL or Hash to query each separated by a newline.",
            nargs="?",
        )
        self._parser.add_argument(
            "-o", "--output", help="Output the results to a file."
        )
        self._parser.add_argument(
            "-b",
            "--bot",
            action="store_true",
            help="Output minimized results for a bot.",
        )
        self._parser.add_argument(
            "-f", "--cef", help="Output the results to a CEF formatted file."
        )
        self._parser.add_argument(
            "-w", "--web", help="Output the results to an HTML file."
        )
        self._parser.add_argument(
            "-c", "--csv", help="Output the results to a CSV file."
        )
        self._parser.add_argument(
            "-d",
            "--delay",
            type=int,
            default=2,
            help="Change the delay to the inputted seconds. Default is 2.",
        )
        self._parser.add_argument(
            "-s",
            "--source",
            help="Run the target against a specific source engine to pull associated domains.\nOptions are defined in the name attribute of the site element in the XML configuration file.\nThis can be a list of names separated by a semicolon.",
        )
        self._parser.add_argument(
            "--proxy", help="Set a proxy to use (eg. proxy.example.com:8080)"
        )
        self._parser.add_argument(
            "-a",
            "--useragent",
            default=f"Automater/{version}",
            help="Set the user-agent seen by web servers being utilized. By default, the user-agent is set to Automater/version",
        )
        self._parser.add_argument(
            "-V",
            "--vercheck",
            action="store_true",
            help="Checks and reports versioning for Automater. Checks each python module in the Automater scope. Default, (no -V) is False",
        )
        self._parser.add_argument(
            "-r",
            "--refreshxml",
            action="store_true",
            help="Refreshes the tekdefense.xml file from the remote GitHub site. Default (no -r) is False.",
        )
        self._parser.add_argument(
            "-v",
            "--verbose",
            action="store_true",
            help="Prints messages to the screen. Default (no -v) is False.",
        )
        self._parser.add_argument(
            "-x", "--xml", help="Path to the folder containing the required XML files"
        )
...
class VersionChecker(object):
    ...
    def getModifiedFileInfo(cls, prefix, gitlocation, filelist):
        ...
        try:
            ...
            return (
                f"There was an error while checking the version of the Automater files. Please see {gitlocation} "
