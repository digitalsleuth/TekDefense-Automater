...
class SitesFile(object):
    ...
    def updateTekDefenseXMLTree(cls, prox, verbose):
        ...
        try:
            ...
            SiteDetailOutput.PrintStandardOutput(
                f"Local file {__TEKDEFENSEXML__} not located. Attempting download.",
                verbose=verbose,
            )
        try:
            if localfileexists:
                ...
                if remotemd5 and remotemd5 != localmd5:
                    SiteDetailOutput.PrintStandardOutput(
                        f"There is an updated remote {__TEKDEFENSEXML__} file at {__REMOTE_TEKD_XML_LOCATION__}. Attempting download.",
                        verbose=verbose,
                    )
            ...
            try:
                SiteDetailOutput.PrintStandardOutput(
                    f"Cannot connect to {__REMOTE_TEKD_XML_LOCATION__}. Server response is {ce.message[0]} Server error code is {ce.message[1][0]}",
                    verbose=verbose,
                )
            except:
                SiteDetailOutput.PrintStandardOutput(
    ...
    def getXMLTree(cls, filename, verbose):
        ...
        if SitesFile.fileExists(filename):
            ...
            SiteDetailOutput.PrintStandardOutput(
                f"No local {filename} file present.", verbose=verbose
            )
        return None
