...
__GITFILEPREFIX__ = (
    "https://raw.githubusercontent.com/digitalsleuth/TekDefense-Automater/master/"
)
...
def main():
    ...
    if parser.hasNoTarget():
        if parser.VersionCheck:
            Site.checkmoduleversion(
                __GITFILEPREFIX__, __GITLOCATION__, parser.Proxy, parser.Verbose
            )
        else:
    ...
    if parser.hasInputFile():
        for tgtstr in TargetFile.TargetList(parser.InputFile, parser.Verbose):
            tgtstrstripped = (
                tgtstr.replace("[.]", ".").replace("{.}", ".").replace("(.)", ".")
            )
