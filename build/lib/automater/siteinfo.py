...
class SiteFacade(object):
    ...
    def runSiteAutomation(
        ...
        if not localsitetree and not remotesitetree:
            print(
                f"Unfortunately there is neither a {__TEKDEFENSEXML__} file nor a {__SITESXML__} file that can be utilized for proper"
            ...
            if localsitetree:
                for siteelement in localsitetree.iter(tag="site"):
                    if self.siteEntryIsValid(siteelement):
                        for targ in targetlist:
                            for source in sourcelist:
                                (
                                    ...
                                    targettype,
                                    target,
                                ) = self.getSiteInfoIfSiteTypesMatch(
                                    source, targ, siteelement
                                )
                                if sitetypematch:
                                    self.buildSiteList(
                                        siteelement,
                        ...
                        print(
                            f"A problem was found in the {__SITESXML__} file. There appears to be a site entry with "
    ...
    def siteEntryIsValid(self, siteelement):
        reportstringcount = len(
            siteelement.find("reportstringforresult").findall("entry")
        )
        sitefriendlynamecount = len(
            siteelement.find("sitefriendlyname").findall("entry")
        )
        regexcount = len(siteelement.find("regex").findall("entry"))
        importantpropertycount = len(
            siteelement.find("importantproperty").findall("entry")
        )

        if (
            reportstringcount == sitefriendlynamecount
            and reportstringcount == regexcount
            and reportstringcount == importantpropertycount
        ):
            return True
        return False
...
class Site(object):
    ...
    def __init__(
        ...
        self._fullURL = ""
        self.FullURL = (
            fullurl  # call the helper method to clean %TARGET% from fullurl string
        )
        ...
        if params is not None:
            self.Params = (
                params  # call the helper method to clean %TARGET% from params string
            )
        self._headers = None
        if headers is not None:
            self.Headers = (
                headers  # call the helper method to clean %TARGET% from params string
            )
        self._postdata = None
        if postdata:
        ...
        self._method = None
        self.Method = (
            method  # call the helper method to ensure result is either GET or POST
        )
        self._results = []
        self._verbose = verbose
    ...
    def checkmoduleversion(self, prefix, gitlocation, proxy, verbose):
        ...
        pythonfiles = [
            os.path.join(execpath, f)
            for f in listdir(execpath)
            if isfile(join(execpath, f)) and f[-3:] == ".py"
        ]
        if proxy:
        ...
        SiteDetailOutput.PrintStandardOutput(
            VersionChecker.getModifiedFileInfo(prefix, gitlocation, pythonfiles),
            verbose=verbose,
        )
    ...
    def buildSiteFromXML(
        ...
        reportstringforresult = Site.buildStringOrListfromXML(
            siteelement, "reportstringforresult"
        )
        sitefriendlyname = Site.buildStringOrListfromXML(
            siteelement, "sitefriendlyname"
        )
        regex = Site.buildStringOrListfromXML(siteelement, "regex")
        fullurl = siteelement.find("fullurl").text
        importantproperty = Site.buildStringOrListfromXML(
            siteelement, "importantproperty"
        )
    ...
    def getImportantProperty(self, index):
        ...
        if isinstance(self._importantProperty, str):
            siteimpprop = getattr(
                self, "get" + self._importantProperty, Site.getResults
            )
        else:
            siteimpprop = getattr(
                self, "get" + self._importantProperty[index], Site.getResults
            )
        return siteimpprop()
...
class SingleResultsSite(Site):
    ...
    def __init__(self, site):
        ...
        """
        self._site = site
        super(SingleResultsSite, self).__init__(
            self._site.URL,
            self._site.WebRetrieveDelay,
            self._site.Proxy,
            self._site.TargetType,
            self._site.ReportStringForResult,
            self._site.Target,
            self._site.UserAgent,
...
class MultiResultsSite(Site):
    ...
    def __init__(self, site):
        ...
        """
        self._site = site
        super(MultiResultsSite, self).__init__(
            self._site.URL,
            self._site.WebRetrieveDelay,
            self._site.Proxy,
            self._site.TargetType,
...
class MethodPostSite(Site):
    ...
    def __init__(self, site):
        ...
        """
        self._site = site
        super(MethodPostSite, self).__init__(
            self._site.URL,
            self._site.WebRetrieveDelay,
            self._site.Proxy,
            self._site.TargetType,
        ...
        self.postMessage(f"{self.UserMessage} {self.FullURL}")
        SiteDetailOutput.PrintStandardOutput(
            f"[-] {self._site.URL} requires a submission for {self._site.Target}. "
