                ...
                if not isinstance(site._regex, str):  # this is a multisite
                    for index in range(
                        ...
                        if sourceurlhasnoreturn:
                            ...
                            if siteimpprop is None or len(siteimpprop) == 0:
                                print(
                                    f"No results in the {site.FriendlyName[index]} category"
                                )
                            else:
                                if (
                                    siteimpprop[index] is None
                                    or len(siteimpprop[index]) == 0
                                ):
                                    print(
                                        f"{site.ReportStringForResult[index]} No results found"
                                    )
                                else:
                                    laststring = ""
                                    ...
                                    if isinstance(siteimpprop[index], str):
                                        if (
                                            f"{site.ReportStringForResult[index]} {str(siteimpprop)}"
                                            != laststring
                                        ):
                                            print(
                                                f'{site.ReportStringForResult[index]} {str(siteimpprop).replace("www.", "www[.]").replace("http", "hxxp")}'
                                            )
                                    ...
                                    # must be a list since it failed the isinstance check on string
                                        ...
                                        for siteresult in siteimpprop[index]:
                                            if (
                                                f"{site.ReportStringForResult[index]} {str(siteresult)}"
                                                != laststring
                                            ):
                                                print(
                                                    f'{site.ReportStringForResult[index]} {str(siteresult).replace("www.", "www[.]").replace("http", "hxxp")}'
                                                )
                    ...
                    if siteimpprop is None or len(siteimpprop) == 0:
                        ...
                        if isinstance(siteimpprop, str):
                            if (
                                f"{site.ReportStringForResult} {str(siteimpprop)}"
                                != laststring
                            ):
                                print(
                                    f'{site.ReportStringForResult} {str(siteimpprop).replace("www.", "www[.]").replace("http", "hxxp")}'
                                )
                                laststring = (
                                    f"{site.ReportStringForResult} {str(siteimpprop)}"
                                )
                        # must be a list since it failed the isinstance check on string
                            ...
                            for siteresult in siteimpprop:
                                if (
                                    f"{site.ReportStringForResult} {str(siteresult)}"
                                    != laststring
                                ):
                                    print(
                                        f'{site.ReportStringForResult} {str(siteresult).replace("www.", "www[.]").replace("http", "hxxp")}'
                                    )
                                    ...
                                    if (
                                        ...
                                        if (
                                            (
                                                "pDNSDATE"
                                                or ("VT" and "Date")
                                                in (site._friendlyName[index]).split()
                                            )
                                            and len(siteimpprop) == 10
                                            and siteimpprop.isdigit()
                                        ):
                                            siteimpprop = datetime.utcfromtimestamp(
                                                int(siteimpprop)
                                            ).strftime("%Y-%m-%d %H:%M:%S UTC")
                                        if site._friendlyName[index] in (
                                            "VT Engine and Result",
                                            "VT pDNSIP",
                                        ) and isinstance(siteimpprop, tuple):
                                        ...
                                        if (
                                            ...
                                            if (
                                                (
                                                    ...
                                                    or ("VT" and "Date")
                                                    in (
                                                        site._friendlyName[index]
                                                    ).split()
                                                )
                                                and len(siteresult) == 10
                                                and siteresult.isdigit()
                                            ):
                                                siteresult = datetime.utcfromtimestamp(
                                                    int(siteresult)
                                                ).strftime("%Y-%m-%d %H:%M:%S UTC")
                                            if site._friendlyName[index] in (
                                                "VT Engine and Result",
                                                "VT pDNSIP",
                                            ) and isinstance(siteresult, tuple):
                                        ...
                                        cefRW.writerow(
                                            ...
                                            + [
                                                ...
                                                + re.sub(
                                                    pattern,
                                                    "",
                                                    site.ReportStringForResult[index],
                                                )
                                                + str(siteimpprop)
                                            ]
                                            + [cef_Severity]
                            ...
                            if f"{tgt}{typ}{source}{res}" != laststring:
                                cefRW.writerow(
                                    ...
                                    + [
                                        ...
                                        + "] "
                                        + re.sub(
                                            pattern, "", site.ReportStringForResult
                                        )
                                        + str(siteimpprop)
                                    ]
                                    + [cef_Severity]
